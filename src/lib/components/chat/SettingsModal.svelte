...
<Modal size="xl" bind:show>
	<div class="text-gray-700 dark:text-gray-100">
		<div class=" flex justify-between dark:text-gray-300 px-5 pt-4 pb-1">
			<div class=" text-lg font-medium self-center">{$i18n.t('User Settings')}</div>
			<button
				class="self-center"
		...
		<div class="flex flex-col md:flex-row w-full px-4 pt-1 pb-4 md:space-x-4">
			...
			{#if visibleTabs.length > 0}
				<!-- Primary tabs: General and Interface -->
				...
				{/if}
				{#if visibleTabs.includes('interface')}
					<button
						class="px-0.5 py-1 min-w-fit rounded-lg flex-1 md:flex-none flex text-left transition {selectedTab ===
						'interface'
							? 'text-[#57068c] dark:text-white'
				...
				<button
					class="px-0.5 py-1 min-w-fit rounded-lg flex-1 md:flex-none flex text-left transition text-gray-500 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"
					on:click={() => {
						showMoreSettings = !showMoreSettings;
					}}
					type="button"
				>
					<div class="self-center mr-2 transition-transform duration-200 {showMoreSettings ? 'rotate-90' : ''}">
					...
					<div class="self-center text-xs">{$i18n.t('More settings')}</div>
				</button>

				<!-- Collapsible secondary tabs (greyed out, unclickable) -->
				<div class="more-settings-panel {showMoreSettings ? 'expanded' : ''}">
					<div class="more-settings-inner">
						<!-- Connections -->
						{#if visibleTabs.includes('connections') && ($user.role === 'admin' || ($user.role === 'user' && $config?.features?.enable_direct_connections))}
							<div class="px-0.5 py-1 min-w-fit rounded-lg flex-1 md:flex-none flex text-left opacity-40 cursor-not-allowed pointer-events-none text-gray-400 dark:text-gray-600">
								<div class="self-center mr-2">
						...
						<!-- Personalization -->
						{#if visibleTabs.includes('personalization')}
							<div class="px-0.5 py-1 min-w-fit rounded-lg flex-1 md:flex-none flex text-left opacity-40 cursor-not-allowed pointer-events-none text-gray-400 dark:text-gray-600">
								<div class="self-center mr-2">
									<User />
								</div>
						...
						<!-- Account -->
						{#if visibleTabs.includes('account')}
							<div class="px-0.5 py-1 min-w-fit rounded-lg flex-1 md:flex-none flex text-left opacity-40 cursor-not-allowed pointer-events-none text-gray-400 dark:text-gray-600">
								<div class="self-center mr-2">
									<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4">
						...
						<!-- Admin Settings -->
						{#if visibleTabs.includes('admin') && $user.role === 'admin'}
							<div class="px-0.5 py-1 min-w-fit rounded-lg flex-1 md:flex-none flex text-left opacity-40 cursor-not-allowed pointer-events-none text-gray-400 dark:text-gray-600">
								<div class="self-center mr-2">
